"""
test_storyboard_seedance_2_0_it2v_filter_15s_data_first_last.py

与 test_storyboard_seedance_2_0_it2v_filter_15s_data.py 相同，
但同时将 first_frame 和 last_frame 两张图作为条件输入 Seedance。

binary_data 传两张图：[first_frame, last_frame]
binary_var_name: ["image", "image"]

结果保存至独立目录 generated_videos_first_last/，CSV 也独立。
"""
...
OUT_ROOT    = LABEL_ROOT / "generated_videos_first_last"
LOG_DIR     = OUT_ROOT / "logs"
CSV_PATH    = OUT_ROOT / "generation_results_first_last.csv"
...
# First / last frames already uploaded to TOS
FIRST_FRAME_TOS_BASE = ("https://tosv.byted.org/obj/dm-stickers-rec-sg"
                        "/tt_template_1400k_15s_video_sample/first_frame")
LAST_FRAME_TOS_BASE  = ("https://tosv.byted.org/obj/dm-stickers-rec-sg"
                        "/tt_template_1400k_15s_video_sample/last_frame")
...
def build_whole_video_prompt(phase0: dict, phase1: dict, phase1_5: dict,
    ...
    parts.append(
        "Reference images: Two reference images are provided. "
        "The FIRST image is the FIRST FRAME of the video — use it as the visual anchor "
        "for the opening scene composition, character appearance, lighting, and setting. "
        "The SECOND image is the LAST FRAME of the video — use it to ensure the ending "
        "scene, final character pose, and closing composition match this target."
    )
...
def get_last_frame_url(vid_label: str) -> str:
    return f"{LAST_FRAME_TOS_BASE}/{vid_label}_last.png"
...
def submit_and_poll(cairo_client, prompt: str, duration: float,
    ...
    """Submit to Seedance with first+last frame, poll until done, return video_url or None."""
    task_input = json.dumps({
        "binary_data": [
            {"data": first_frame_url, "type": "image"},
            {"data": last_frame_url,  "type": "image"},
        ],
        "req_json": {
            "prompt": prompt,
            "language": "zh",
...
if __name__ == "__main__":
    ...
    for rec in all_records:
        ...
        last_frame_url  = get_last_frame_url(vid_label)
        ...
        for lv in PROMPT_LEVELS:
            ...
            video_url = submit_and_poll(cairo_client, prompt, duration,
                                        first_frame_url, last_frame_url, logger)
            if not video_url:
                continue
